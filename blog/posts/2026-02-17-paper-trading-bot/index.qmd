...
description: "Built a real-time paper trading bot to forward-validate the multi-factor pipeline. Architecture: WebSocket price feeds → signal generation → simulated execution with realistic fill modeling. Also: Polymarket dropped fees to 0/0 bps — this changes everything."
title-block-banner: false
---

## The Gap Between Backtest and Reality

Day 6 gave us a backtest: 14 trades, 57.1% win rate, +0.12% edge per trade with maker orders. But $n = 14$ is noise. We need 100+ trades for statistical significance, and backtests are inherently optimistic — they can't capture slippage, latency, orderbook dynamics, or the psychological pressure of watching real prices move.

The standard quant workflow is: **backtest → paper trade → small live → scale**. Today we build step 2.

But first, a breaking development that changes the entire economics of this strategy.
